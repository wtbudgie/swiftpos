...
type EditMenuSectionProps = {
	restaurantData: SanitizedRestaurant;
};

export default function EditMenuSection({ restaurantData }: EditMenuSectionProps) {
						...
						<div key={category.id} id={category.id} className="transition-opacity duration-500 ease-in opacity-0 animate-fadeIn">
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
								{categoryItems.map((item) => (
									<ItemCard key={item.id} item={item} onEditClick={openItemModal} />
								))}
							</div>
						</div>
					);
